...
### Personal Graph 2.0 (Patch 28)
The Personal Graph now populates the previously reserved Person, Decision, and Event node types from explicit structured facts authored inside Notes. Supported markers include `Person:` / `People:`, `Decision:`, and `Event:` entries, with optional explicit event dates. Ordinary prose is never treated as entity data. JotCue derives Note-to-entity relationships and may propagate those relationships to existing Tasks and Projects only when the connection is deterministic and unambiguous. The graph remains an in-memory read model: Notes and existing planning objects remain the sources of truth, and no separate graph dataset is persisted or synchronized.

### Contextual Reasoning 2.0 (Patch 29)
Ask JotCue can now combine current planning state instead of answering focus questions from isolated lists. A deterministic contextual-reasoning layer evaluates actionable Tasks against deadlines, priorities, Project urgency, effort estimates, dependencies and `Waiting for` blockers, accepted schedule blocks, free capacity, replanning signals, downstream dependency impact, Personal Graph relationships, and current local time. The result identifies a best next action, explains why it matters, surfaces relevant downstream or graph context, suggests a realistic execution window when one is known, and may provide an actionable alternative with confidence and limitations. This layer is read-only: it does not add new mutation authority or autonomous execution.

### Proactive Pulse 2.0 (Patch 30)
Pulse now turns contextual reasoning into concise live interventions instead of relying mainly on counts. Morning Pulse can surface the strongest next action, explain why it matters, show a deterministic execution window when one is known, distinguish blocked work that can safely stay out of focus, and surface explicit recovery guidance when planned work slips. Daily Closing uses the same current planning context to help close unresolved work and identify what still needs a new home. The proactive layer remains read-only and local-first: it reuses existing contextual reasoning, scheduling, dependency, replanning, and Personal Graph state, and it does not add autonomous execution authority. Recurring OS Morning Pulse and Daily Closing notifications remain deliberately generic so stale recommendations are not repeated on future days.
